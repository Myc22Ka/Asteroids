...
void Invader::update(float deltaTime)
{
    ...
    if (!shoot.isEffectActive()) {
        shoot.startEffect(3.0f);

        Game::addEntity(new EnemyBullet(position, directionToPlayer, angle, Sprites::HOMING_BULLET));
    ...
    if (physics::distance(newPosition, position) <= 100.0f) return;
